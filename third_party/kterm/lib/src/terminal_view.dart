...
// PATCH (ssh_agent): 输入链路调试日志（与 custom_text_edit 共用文件）
void _debugLog(String msg) {
  try {
    final f = File('${Directory.systemTemp.path}/ssh_agent_term.log');
    f.writeAsStringSync(
        '[${DateTime.now().toIso8601String()}] [kterm-view] $msg\n',
...
class TerminalViewState extends State<TerminalView> {
  ...
  void _onInsert(String text) {
    _debugLog('_onInsert text="${text.length > 60 ? '${text.substring(0, 60)}…' : text}" '
        'len=${text.length}');
  ...
  KeyEventResult _handleKittyKeyEvent(KeyEvent event) {
    ...
    if (event is KeyDownEvent || event is KeyRepeatEvent) {
      ...
      if (!useKittyEncoding) {
        ...
        _debugLog('_handleKeyEvent bare-key -> ignored (expect TextInput)');
        return KeyEventResult.ignored;
      }
  ...
  KeyEventResult _handleKeyEvent(FocusNode focusNode, KeyEvent event) {
    _debugLog('_handleKeyEvent ${event.runtimeType} '
    ...
    // PATCH (ssh_agent): Flutter Windows 引擎的 TextInput 通道在此环境
    // 整体失效（updateEditingValue 全局零回调，TextField 同样受影响），
    // 而 KeyEvent.character 实测非空。因此直接用 character 补发字符，
    // 绕过失效的 IME 通道；Ctrl/Alt 组合键除外（走下方 keytab 分支）。
    if (event is KeyDownEvent) {
      final ch = event.character;
      if (ch != null && ch.isNotEmpty) {
        final keyboard = HardwareKeyboard.instance;
        if (!keyboard.isControlPressed && !keyboard.isAltPressed) {
          _debugLog('_handleKeyEvent PATCH textInput char="$ch"');
    ...
    _debugLog('_handleKeyEvent result=${handled ? 'handled' : 'ignored'} '
        'key=$key');
