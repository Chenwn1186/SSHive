...
namespace flutter_inappwebview_plugin
{
  ...
  public:
    ...
    // ssh_agent patch: 运行时热更新滚轮倍率（不重建 WebView）
    void setScrollMultiplier(double v);
  ...
  private:
    ...
    // ssh_agent patch: 滚轮倍率的动态成员（初始化时取自 settings，
    // setScrollMultiplier 运行时更新，sendScroll 读取它而不是 settings）
    double scrollMultiplier_ = 1;
