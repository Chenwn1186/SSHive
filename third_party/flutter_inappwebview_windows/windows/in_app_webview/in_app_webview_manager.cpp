...
namespace flutter_inappwebview_plugin
{
  ...
  // ssh_agent patch: 热更新所有 WebView 的滚轮倍率（含 keepAlive 存活实例）
  void InAppWebViewManager::setScrollMultiplierAll(double v)
  {
    for (auto& [id, platformView] : webViews) {
      if (platformView && platformView->view) {
        platformView->view->setScrollMultiplier(v);
      }
    }
    for (auto& [id, platformView] : keepAliveWebViews) {
      if (platformView && platformView->view) {
        platformView->view->setScrollMultiplier(v);
      }
    }
